...
type SrienterpricesDataChaincode struct {
}

// CryptocoinData represents a standard token implementation
type SrienterpricesData struct {
...
func (token *SrienterpricesDataChaincode) Init(stub shim.ChaincodeStubInterface) peer.Response {
...
// Invoke method
func (token *SrienterpricesDataChaincode) Invoke(stub shim.ChaincodeStubInterface) peer.Response {
...
func AddData(stub shim.ChaincodeStubInterface, args []string) peer.Response {
	...
	PlaceOfArrival := args[8]

	data := SrienterpricesData{DocType: docType, BatchId: Batchid, LogisticsId: Logisticsid, FarmersId: Farmersid, ProductId: Productid, ProductName: Productname, Quantity: quantity, DateOfArrival: DateOfArrivalConverted, PlaceOfArrival: PlaceOfArrival}
...
func main() {
	...
	err := shim.Start(new(SrienterpricesDataChaincode))
	if err != nil {
