...
if [ "$FABRIC_CFG_PATH" == "" ]; then 
   ...
   echo "> source  set-env.sh  farmersorg"
   echo "> source  set-env.sh  logistics"
   echo "> source  set-env.sh  srienterprices"
   echo "> source  set-env.sh  priceauthority"
   exit
fi
...
function update_properties {
    ...
    case  "$CC2_ENDORSING_PEERS" in
        "auto")
            ...
            ;;
        "farmersorg")
            CC2_ENDORSING_PEERS="--peerAddresses=farmersorg-peer1.farmersorg.com:6051"
            ;;
        "logistics")
            CC2_ENDORSING_PEERS="--peerAddresses=logistics-peer1.logistics.com:7051"
            ;;
        "srienterprices")
            CC2_ENDORSING_PEERS="--peerAddresses=srienterprices-peer1.srienterprices.com:8051"
            ;;
        "priceauthority")
            CC2_ENDORSING_PEERS="--peerAddresses=priceauthority-peer1.priceauthority.com:9051"
            ;;        
        "both")
            CC2_ENDORSING_PEERS="--peerAddresses=farmersorg-peer1.farmersorg.com:6051 --peerAddresses=logistics-peer1.logistics.com:7051 --peerAddresses=srienterprices-peer1.srienterprices.com:8051 --peerAddresses=priceauthority-peer1.priceauthority.com:9051"
            ;;
        *)
            CC2_ENDORSING_PEERS="--peerAddresses=$CORE_PEER_ADDRESS"
    esac
...
function cc_approveformyorg {
    ...
    case "$ORGANIZATION_CONTEXT" in
        farmersorg) 
              APPROVAL_DONE=$COMMITTED_APPROVAL_FARMERSORG
              ;;
        logistics) 
              APPROVAL_DONE=$COMMITTED_APPROVAL_LOGISTICS
              ;;
        srienterprices) 
              APPROVAL_DONE=$COMMITTED_APPROVAL_SRIENTERPRICES
              ;;
        priceauthority) 
              APPROVAL_DONE=$COMMITTED_APPROVAL_PRICEAUTHORITY
              ;;            
    esac
...
function cc_commit {
    ...
    cmd="peer lifecycle chaincode commit -C $CC_CHANNEL_ID -n $CC_NAME -v $CC_VERSION \
        ...
        # --peerAddresses=priceauthority-peer1.priceauthority.com:9051 --peerAddresses=srienterprices-peer1.srienterprices.com:8051 --peerAddresses=logistics-peer1.logistics.com:7051  --peerAddresses=farmersorg-peer1.farmersorg.com:6051"
