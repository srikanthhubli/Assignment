...
if [ -z $2 ];
then
    echo "Switching PEER_NAME for Org = farmersorg-peer1"
    PEER_NAME=farmersorg-peer1
else
    PEER_NAME=$2
fi
...
PORT_NUMBER_BASE=6050
if [ -z $4 ]
then
    echo "Setting PORT_NUMBER_BASE=6050"   
else
    PORT_NUMBER_BASE=$4
fi
...
if [ -z $5 ]
then
    echo "Setting ORDERER_ADDRESS=orderer.farmersorg.com:6050"   
